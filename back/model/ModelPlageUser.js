...
class ModelPlageUser extends Model {
  ...
  static async getAllUsers () {
    ...
    try {
      const sql = 'SELECT * FROM userplage up, userrole ur WHERE up.role_id=ur.role_id;'
  ...
  static async logPlage (email, password) {
    ...
    const goodPwd = await comparePassword(password, user.salt, user.password)
    if (goodPwd) {
      if (user.enabled) {
        return user
      } else {
  ...
  static async getSkills (user_id, locale) {
    ...
    if (result && result.rows) {
      ...
      for (let i = 0; i < result.rows.length; i++) {
        const origin_row = result.rows[i]
        let translated_row

        if (origin_row.locale != locale) {
          console.log(origin_row)
          if (origin_row.locale == 'en') {
            const tmp = await client.query('SELECT skill_code, name AS skill_name FROM skill WHERE ref_code = $1 AND locale = $2', [origin_row.skill_code, locale])
            if (tmp && tmp.rows && tmp.rows.length > 0) translated_row = tmp.rows[0]
          } else {
            console.log(origin_row.locale)
            const tmp = await client.query('SELECT skill_code, name AS skill_name FROM skill WHERE skill_code = $1 AND locale = $2', [origin_row.ref_code, locale])
            if (tmp && tmp.rows && tmp.rows.length > 0) translated_row = tmp.rows[0]
          }
          console.log(translated_row)
        }

        const row = translated_row || origin_row
  ...
  static async getUserRole (id) {
    ...
    const client = await ModelDB.connect_to_db_from_pool()
    try {
      const sql = 'SELECT userrole.name FROM ' + ModelPlageUser.dbName + ', userrole WHERE userplage.user_id = $1 AND userplage.role_id = userrole.role_id;'
...
async function comparePassword (password, salt, hash) {
  ...
  const passwordData = await hashSHA512(password, salt)
