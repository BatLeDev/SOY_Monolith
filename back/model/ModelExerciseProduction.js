...
class ModelExerciseProduction extends Model {
  ...
  static async evaluateStudProduction (ps_id, ex_id, user_id, data_file, callback) {
    ...
    const fData = JSON.parse(data_file)
    // debug("fData = "+fData)
    debug('evaluateStudProd receives submitted file = ' + fData.name)
    let fAnalyzePyScript
    let fTemplateArchive
    let localeEX

    const personalizedHtmlStatement = undefined
    const personalizedQuestionArchive = undefined
    ...
    const req = {}; req.params = {}; req.params.exerciseId = ex_id
    ...
    if (exercise != null) {
      ...
      const fSubmittedAnswerPath = path + 'submitted.tar.gz'
      const fAnalyzeScriptPath = path + fAnalyzePyScript.name
                              ...
                              fs.copyFile(
                                ...
                                function (err) {
                                  if (!err) {
                                    ...
                                    const py_script = fAnalyzePyScript.name
  ...
  static async evaluateStudProductionLMS (ex_id, archRep, lmsStudentStatement, callback) {
    debug('Preparing to launch analysis of exercise production')
    const fData = JSON.parse(archRep) // fichier soumis par l'étudiant
