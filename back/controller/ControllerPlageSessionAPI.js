...
module.exports.readAll = async function (req, res) {
  ...
  for (let i = 0; i < tabSession.length; i++) {
    const author = await ModelPlageUser.read(tabSession[i].author)
    tabSession[i].author = {
  ...
  for (let i = 0; i < toreturn.exercises.length; i++) {
    ...
    for (let j = 0; j < productions.length; j++) {
      if (!user_prods[productions[j].user_id]) {
        const user = await ModelPlageUser.read(productions[j].user_id)
...
module.exports.create = async function (req, res) {
  ...
  const sequence = await ModelSequence.read(req.body.seq_id)
  if (dateOK !== true) {
...
module.exports.update = async function (req, res) {
  ...
  const data = db_session // starting from info in db avoiding forgery)
  ...
  const up_session = new ModelPlageSession(data)
  ...
  if (success == true) {
    ...
    const author = await ModelPlageUser.read(new_session.author)
    new_session.author = {
      user_id: author.user_id,
      lastname: author.lastname,
      firstname: author.firstname
