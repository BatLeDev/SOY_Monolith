...
module.exports.create = async function (req, res) {
  ...
  const knowEmail = await ModelPlageUser.checkEmailinDB(req.body.email)
  if (!knowEmail) {
    ...
    const passData = await ModelPlageUser.saltPepperHashPassword(req.body.password)
    const data = {
      user_id: undefined,
...
module.exports.getSkills = async function (req, res) {
  ...
  const skills = await ModelPlageUser.getSkills(userId, locale)
...
module.exports.verify = async function (req, res) {
  ...
  if (req.headers.cookie) {
    ...
    cookies.forEach(async (cookie) => {
      ...
      if (key === 'refreshToken') {
        foundRefresh = true
        const refreshToken = cookieArray[1]
        try {
          ...
          const token = jwt.sign(data, process.env.SECRET_JWT, { expiresIn: accessTokenExpiration })
          tokenList[refreshToken] = { access_token: token, refreshToken }
          res.cookie('access_token', token, { maxAge: accessTokenExpiration, httpOnly: true, SameSite: 'Strict' }).status(200).json(data)
        } catch (err) {
...
module.exports.requestPasswordReset = async function (req, res) {
  ...
  if (knownEmail) {
    const user = await ModelPlageUser.getUserByEmailId(req.body.email)

    // Change nonce for user wanting to change passwd
    const nonce = await ModelPlageUser.genRandomSalt(128)

    // Update db to reflect change
    await ModelPlageUser.updateNonce(user.user_id, nonce)
  ...
  if (userByNonce !== 'none') {
    ...
    if (user) {
      if (req.body.pwd === req.body.pwd2) {
        const passData = await ModelPlageUser.saltPepperHashPassword(req.body.pwd)
        const data = {
          user_id: user.user_id,
          lastname: user.lastname,
          firstname: user.firstname,
        ...
        const newUser = new ModelPlageUser(data)
        const id = await newUser.update()
        if (id) {
