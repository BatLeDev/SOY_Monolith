...
module.exports.list = async function (req, res) {
  ...
  if (user) {
    let filename = ''
    if (user.role_id === 2) { filename = 'teacher' } else if (user.role_id === 3) { filename = 'student' } else { filename = 'admin' } // note: no help for admin at the moment
    const urlFile = 'static/files/help/' + filename + '.' + req.i18n.getLocale() + '.html'
